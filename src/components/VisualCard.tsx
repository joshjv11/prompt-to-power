...
import { Visual, DataRow, VisualType } from '@/store/appStore';
import { cn } from '@/lib/utils';
import { TrendingUp, TrendingDown, Activity, BarChart3 } from 'lucide-react';
import { useDrillThrough, applyDrillFilters, DrillFilter } from '@/hooks/useDrillThrough';
...
interface VisualCardProps {
  ...
  index: number;
  filters: DrillFilter[];
  onChartClick: (filter: DrillFilter) => void;
}

// Loading skeleton for charts
const ChartSkeleton = memo(() => (
  <div className="h-[220px] space-y-3 p-4">
    <Skeleton className="h-4 w-1/3" />
    <div className="flex items-end gap-2 h-[160px]">
      {[0.6, 0.8, 0.4, 0.9, 0.7, 0.5].map((h, i) => (
...
const VisualCardComponent = ({ visual, data, index, filters, onChartClick }: VisualCardProps) => {
  ...
  const chartData = useMemo(() => {
    if (!isVisible || filteredData.length === 0) return [];
  ...
  }, [visual.dimensions, visual.id, onChartClick]);
      ...
      case 'waterfall':
        ...
        const waterfallData = chartData.map((item) => {
          ...
          return {
            ...
            fill: item.value >= 0 ? COLORS[2] : COLORS[4]
          };
        });
...
export const VisualCard = memo(VisualCardComponent, (prevProps, nextProps) => {
  // Only re-render if these specific props change
  return (
    prevProps.visual.id === nextProps.visual.id &&
